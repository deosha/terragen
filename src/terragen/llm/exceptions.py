...
class APIError(LLMError):
    ...
    def __init__(
        self, message: str, provider: str = "unknown", status_code: int | None = None
    ):
...
class NoAvailableProviderError(LLMError):
    ...
    def __init__(
        self, errors: list[tuple[str, Exception]], unavailable: list[str] | None = None
    ):
        self.errors = errors
        ...
        if self.unavailable:
            ...
            missing = [
                f"{p} ({env_vars.get(p, p.upper() + '_API_KEY')})"
                for p in self.unavailable
            ]
